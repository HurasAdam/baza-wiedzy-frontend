...
interface IssueReportsModalProps {
  isIssueReportsModalOpen: boolean;
  setIsIssueReportsModalOpen: (isSettingsModalOpen: boolean) => void;
...
export const IssueReportsModal = ({
  isIssueReportsModalOpen,
  setIsIssueReportsModalOpen,
  closeOnOutsideClick = false,
}: IssueReportsModalProps) => {
  const [activeTab, setActiveTab] = useState("account");

  return (
    <Dialog open={isIssueReportsModalOpen} onOpenChange={setIsIssueReportsModalOpen} modal>
      <DialogContent
        {...(!closeOnOutsideClick ? { onInteractOutside: (e) => e.preventDefault() } : {})}
        className="max-h-[83vh] min-h-[90vh] min-w-[88vw] md:min-w-[75vw] xl:min-w-[74vw] flex flex-col p-0 gap-0"
      >
        {/* HEADER */}
          ...
          <Card className="flex flex-grow flex-col w-full overflow-hidden border-none rounded-none p-0 bg-background">
            <CardContent className="flex flex-grow h-full p-0 overflow-hidden">
              ...
              <section className="w-full py-6 px-7 overflow-y-auto h-full scrollbar-custom">
                ...
                {activeTab === "account" && <div>1</div>}
                {activeTab === "profile" && <div>2</div>}
                {activeTab === "theme" && <div>3</div>}
