...
import { useCreatePinnedLinkMutation } from "../../hooks/pinned-links/use-pinned-links";
...
export const PinnedLinkModal = ({ isOpen, setIsOpen }: UsefulLinkModalProps) => {
  const { userData } = useOutletContext<{ onOpenCreateIssueReport: () => void; userData: AuthUserData }>();
  const { mutate: createPinnedLinkMutate, isPending: isCreateUsefulLinkLoading } = useCreatePinnedLinkMutation();
          ...
          toast.warning(
            <div
              style={{
                ...
                gap: "12px",
                fontFamily: "Inter, sans-serif",
                color: "#b45309", // np. pomarańczowy / amber
                fontSize: "14px",
              }}
            >
              <div>
      ...
      <DialogContent className="max-h-[80vh] overflow-y-auto">
        <div className="space-y-2 pb-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <DialogTitle className="text-lg font-semibold leading-none">Przypnij link</DialogTitle>
          </div>

          <p className="text-sm text-muted-foreground max-w-[420px]">
            Przypnij link do zasobu na swojej tablicy, aby mieć do niego szybki dostęp.
          </p>
        </div>
