...
const IssueReportSelector = ({ onSelect }: { onSelect: (type: "bug" | "proposal") => void }) => (
  <div className="flex flex-col items-center justify-center h-full p-12 bg-background">
    <h2 className="text-3xl font-bold text-muted-foreground mb-12 tracking-tight">Wybierz rodzaj zgłoszenia</h2>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center p-8">
          <div className="p-4 bg-rose-300/10 rounded-full mb-4 transition-transform duration-300 group-hover:scale-110">
            <Bug size={44} className="text-rose-600 transition-colors duration-300 group-hover:text-rose-700" />
          </div>
          <CardContent className="space-y-6 relative z-10">
            <h3 className="text-2xl font-semibold text-foreground">Zgłoś błąd</h3>
            <p className="text-muted-foreground text-sm leading-relaxed">Aby ułatwić rozwiązanie problemu, podaj:</p>
            <div className="rounded-lg p-4 space-y-3 mt-2">
              {["Kroki prowadzące do błędu", "Oczekiwane zachowanie", "Aktualny rezultat"].map((item, idx) => (
            ...
            <Button variant="outline" className="mt-4 border-rose-500/25 text-rose-500 hover:bg-transparent">
              Wyślij zgłoszenie
            </Button>
          </CardContent>
        </div>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center p-8">
          <div className="p-4 bg-amber-300/10 rounded-full mb-4 transition-transform duration-300 group-hover:scale-110">
            <Lightbulb size={44} className="text-amber-600 transition-colors duration-300 group-hover:text-amber-700" />
          </div>
          <CardContent className="space-y-6 relative z-10">
            <h3 className="text-2xl font-semibold text-foreground">Zgłoś propozycję</h3>
            <p className="text-muted-foreground text-sm leading-relaxed">
              Chcesz zaproponować nowe rozwiązanie? Podaj:
            </p>
            <div className=" rounded-lg p-4 space-y-3 mt-2">
              {["Opis funkcji lub usprawnienia", "Korzyści dla użytkowników", "Przykład zastosowania"].map(
                (item, idx) => (
                  <div key={idx} className="flex items-start gap-2 text-sm text-foreground">
                    <svg
            ...
            <Button variant="outline" className="mt-4 border-amber-500/25 text-amber-500">
              Wyślij pomysł
            </Button>
          </CardContent>
        </div>
