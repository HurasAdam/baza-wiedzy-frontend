...
interface AttachmentHistoryItem {
  changes?: {
    field: string;
    oldValue: AttachmentChange;
    newValue: { removed: boolean };
  }[];
  createdBy?: {
    _id: string;
    name: string;
...
export const ArticleDetailsAttachmentRemoved: React.FC<Props> = ({ item }) => {
  if (!item) return <div className="text-center text-muted-foreground py-6">Brak danych o tym wpisie historii</div>;

  const change = item.changes?.find((c) => c.field === "attachments");
  const attachment = change?.oldValue;
  ...
  const userName = item.createdBy ? `${item.createdBy.name} ${item.createdBy.surname}` : "System";

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {/* Karta 1: Szczegóły usuniętego załącznika */}
      <Card className="py-5.5 px-6 bg-card border border-border rounded-xl shadow-sm hover:shadow-lg transition-all flex flex-col gap-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-semibold text-foreground">Załącznik został usunięty</h2>
        </div>
      ...
      <Card className="py-5.5 px-6 bg-card border border-border rounded-xl shadow-sm hover:shadow-lg transition-all flex flex-col gap-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-semibold text-foreground">Akcja wykonana przez</h2>
        </div>
